...
import { YStack, XStack, Text, Button, H5, Card, ScrollView, Image, useTheme } from "tamagui";
import MapView, { Marker, UrlTile, Callout, Polyline } from "react-native-maps";
import { Footprints, MapPin, ChevronLeft, Store, ChevronRight } from "@tamagui/lucide-icons";
...
const EventDetail = ({ route, navigation }) => {
  ...
  const challengeRouteCoordinates = nearbySponsors.length > 0 ? [
    { latitude: location.latitude, longitude: location.longitude },
    ...
    // Add last checkpoint
    {
      latitude: location.checkpoints[location.checkpoints.length - 1].latitude,
      longitude: location.checkpoints[location.checkpoints.length - 1].longitude
    },
    { latitude: location.latitude, longitude: location.longitude }
  ] : [];

  // Calculate points and totals for both routes
  const mainRoutePoints = location.checkpoints
    .reduce((sum, cp) => sum + cp.points, 0);
  const mainRouteSteps = location.checkpoints
    .reduce((sum, cp) => sum + cp.steps, 0);
  const mainRouteDistance = location.checkpoints
    .reduce((sum, cp) => sum + cp.approx_distance, 0);
            ...
            <XStack ai="center" space="$2">
              <TouchableOpacity onPress={() => navigation.goBack()}>
                <XStack ai="center" space="$2"> 
                  ...
                  <H5 p="$0" color="$color">{location.name}</H5>
                </XStack>
              </TouchableOpacity>
                  ...
                  <YStack space="$3" width="100%">
                    <XStack space="$3" ai="center" width="100%">
                      ...
                      <YStack f={1}>
                        ...
                        <Text fontSize={14} color={theme.cyan8.val} fontWeight="bold">
              ...
              <YStack space="$3" width="100%">
                  ...
                  <YStack space="$2" width="100%">
                    <XStack space="$2" ai="center">
                      <View style={[styles.routeIndicator, { backgroundColor: theme.magenta7.val }]} />
                      <Text color="$color" fontSize={14} fontWeight="bold">Quick Route</Text>
                    </XStack>
                    <XStack space="$4" pl="$4" jc="space-between" width="100%">
                      <XStack space="$2" ai="center">
                        <Footprints size={16} color={theme.magenta7.val} />
                        <Text color="$color" fontSize={14}>{mainRouteSteps} steps</Text>
                      </XStack>
                      <XStack space="$2" ai="center">
                        <MapPin size={16} color={theme.magenta7.val} />
                        <Text color="$color" fontSize={14}>{mainRouteDistance.toFixed(1)} km</Text>
                      </XStack>
                      <XStack space="$2" ai="center">
                        <Text color={theme.magenta7.val} fontSize={14} fontWeight="bold">{mainRoutePoints} pts</Text>
                      </XStack>
                    </XStack>
                  </YStack>
                  ...
                  {challengeRouteCoordinates.length > 0 && (
                    <YStack space="$2" width="100%">
                      <XStack space="$2" ai="center">
                        <View style={[styles.routeIndicator, { backgroundColor: theme.cyan8.val }]} />
                        <Text color="$color" fontSize={14} fontWeight="bold">Challenge Route</Text>
                      </XStack>
                      <XStack space="$4" pl="$4" jc="space-between" width="100%">
                        <XStack space="$2" ai="center">
                          <Footprints size={16} color={theme.cyan8.val} />
                          <Text color="$color" fontSize={14}>{Math.round(challengeRouteSteps)} steps</Text>
                        </XStack>
                        <XStack space="$2" ai="center">
                          <MapPin size={16} color={theme.cyan8.val} />
                          <Text color="$color" fontSize={14}>{challengeRouteDistance.toFixed(1)} km</Text>
                        </XStack>
                        <XStack space="$2" ai="center">
                          <Text color={theme.cyan8.val} fontSize={14} fontWeight="bold">{challengeRoutePoints} pts</Text>
                        </XStack>
                      </XStack>
                      <Text color="$color" fontSize={12} opacity={0.7} pl="$4">
                        Includes {nearbySponsors.length} sponsor{nearbySponsors.length > 1 ? 's' : ''} (+{nearbySponsors.length * 50} bonus points)
                      </Text>
                    </YStack>
                  )}
                </YStack>
                      ...
                      <Card.Header padded p="$2" width="100%">
                        <YStack space="$1" width="100%">
                          ...
                          <XStack space="$3" jc="space-between" width="100%">
                            <XStack space="$2" ai="center">
                              <Text color={theme.cyan7.val} fontWeight="bold" fontSize={12}>
                                {checkpoint.points} points
                              </Text>
                            </XStack>
                            <XStack space="$2" ai="center">
                              <Footprints size={14} color={theme.magenta7.val} />
                              <Text color="$color" fontSize={12}>{checkpoint.steps} steps</Text>
              ...
              <Card.Footer padded p="$3" width="100%">
                <YStack space="$3" width="100%">
                  <XStack space="$4" jc="space-between" width="100%">
                    <YStack>
                      <Text color="$color" opacity={0.7} fontSize={12}>Date</Text>
                      <Text color="$color" fontSize={16} fontWeight="bold">{location.date}</Text>
                    </YStack>
                    <YStack>
                      <Text color="$color" opacity={0.7} fontSize={12}>Start Time</Text>
                      <Text color="$color" fontSize={16} fontWeight="bold">{location.startTime}</Text>
                    </YStack>
                  </XStack>
